...
const years: { year: number; people: AlumniItem[] }[] = [
  ...
  {
    year: 2024,
    people: Array.from({ length: 6 }).map((_, i) => ({
      name: `Alumni 2024 - ${i + 1}`,
    })),
  },
  {
    year: 2023,
    people: Array.from({ length: 6 }).map((_, i) => ({
      name: `Alumni 2023 - ${i + 1}`,
    })),
  },
  {
    year: 2022,
    people: Array.from({ length: 6 }).map((_, i) => ({
      name: `Alumni 2022 - ${i + 1}`,
    })),
  },
  {
    year: 2021,
    people: Array.from({ length: 6 }).map((_, i) => ({
      name: `Alumni 2021 - ${i + 1}`,
    })),
  },
  {
    year: 2020,
    people: Array.from({ length: 6 }).map((_, i) => ({
      name: `Alumni 2020 - ${i + 1}`,
    })),
  },
];
    ...
    <div className="relative min-h-screen bg-gradient-to-b from-[#071021] via-[#081226] to-[#04121a]">
      <div className="relative max-w-7xl mx-auto px-4 py-8 sm:py-12 md:py-16">
        <AnimatedHeading
          text="ALUMNI"
          level="h1"
          className="text-white text-center text-2xl sm:text-3xl md:text-4xl mb-2"
        />
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
              ...
              >
                {group.people.map((p) => (
                  ...
                  >
                    ...
                    <div className="font-semibold text-white text-center mb-1 text-sm sm:text-base">
                      {p.name}
                    </div>
                    <div className="text-xs sm:text-sm text-slate-400 text-center mb-4">
                      {p.role ?? "Alumni"}
                    </div>
