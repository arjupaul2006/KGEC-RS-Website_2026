		...
		<div className="relative min-h-screen bg-gradient-to-b from-[#071021] via-[#081226] to-[#04121a]">
			<div className="relative max-w-7xl mx-auto px-4 py-16">
				<motion.div
					...
					className="mb-12"
				>
					<AnimatedHeading text="Admin Panel" level="h2" className="text-white mb-2" />
					<motion.p
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
				...
				>
					{[
						...
						<motion.div
							...
							className="bg-white/2 border border-white/6 rounded-2xl shadow-lg p-6 hover:border-cyan-400/50 transition-all duration-300 backdrop-blur-sm"
						>
							<div className="font-semibold text-white text-lg mb-2">{item.title}</div>
							<p className="text-slate-300 text-sm mb-4">{item.desc}</p>
							<button className="w-full bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-bold py-2 px-4 rounded-lg hover:shadow-lg hover:shadow-cyan-500/50 transition-all duration-300">
								Manage
							</button>
						</motion.div>
					))}
					...
					>
						{[
							...
							<motion.div
								...
								className="bg-white/2 border border-white/6 rounded-xl p-4 text-center hover:border-white/12 transition-all duration-300 backdrop-blur-sm"
							>
								<div className={`text-3xl font-bold bg-gradient-to-r ${stat.color} to-white/80 bg-clip-text text-transparent mb-2`}>{stat.value}</div>
