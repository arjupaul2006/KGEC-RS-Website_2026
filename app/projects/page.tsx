...
export default function Projects() {
  ...
  const currentProjects = [
    // Array of project objects with relevant details
    {
      name: "Project Alpha",
      status: "Ongoing",
      year: "2024",
  ...
  const pastProjects = [
    // Array of past project objects with relevant details
    {
      name: "Project Alpha",
      status: "Ongoing",
      year: "2024",
  ...
  useEffect(() => {
    const startCurrent = () => {
      currentAutoRef.current = setInterval(() => {
        ...
        if (scrollLeft >= max)
          currentRef.current.scrollTo({ left: 0, behavior: "smooth" });
        else currentRef.current.scrollBy({ left: 360, behavior: "smooth" });
      }, 1800);
    };
    const stopCurrent = () => {
      if (currentAutoRef.current) {
        clearInterval(currentAutoRef.current);
        currentAutoRef.current = null;
      }
    };
    ...
    const startPast = () => {
      pastAutoRef.current = setInterval(() => {
        ...
        const max = scrollWidth - clientWidth;
        if (scrollLeft >= max)
          pastRef.current.scrollTo({ left: 0, behavior: "smooth" });
        else pastRef.current.scrollBy({ left: 360, behavior: "smooth" });
      }, 2200);
    };
    const stopPast = () => {
      if (pastAutoRef.current) {
        clearInterval(pastAutoRef.current);
        pastAutoRef.current = null;
      }
    };
    ...
    return () => {
      ...
      stopPast();
      if (elCurrent) {
        elCurrent.removeEventListener("mouseenter", stopCurrent);
        elCurrent.removeEventListener("mouseleave", startCurrent);
      }
      if (elPast) {
        elPast.removeEventListener("mouseenter", stopPast);
        elPast.removeEventListener("mouseleave", startPast);
      }
    };
  }, []);
  return (
      ...
      <div className="relative max-w-7xl mx-auto px-4 py-8 sm:py-16">
        ...
        <div className="mt-16">
          ...
          <div className="relative group">
            ...
            <div
              ref={currentRef}
              className="flex gap-4 sm:gap-6 overflow-x-auto pb-2 scroll-smooth"
            >
              {currentProjects.map((cp, i) => (
                <motion.div
                  key={i}
                ...
                >
                  <div
                    className={`h-36 sm:h-40 bg-gradient-to-br ${i % 3 === 0 ? "from-cyan-500/20 to-blue-500/20" : i % 3 === 1 ? "from-blue-500/20 to-purple-500/20" : "from-green-500/20 to-cyan-500/20"} rounded-lg border border-white/10`}
                  />
          ...
          <div className="relative group">
            ...
            >
              {pastProjects.map((pp, i) => (
                ...
                >
                  <div
                    className={`h-32 sm:h-36 bg-gradient-to-br ${i % 3 === 0 ? "from-cyan-500/20 to-blue-500/20" : i % 3 === 1 ? "from-blue-500/20 to-purple-500/20" : "from-green-500/20 to-cyan-500/20"} rounded-lg border border-white/10`}
                  />
                  <div className="mt-4 font-semibold text-white text-sm sm:text-base">
                    {pp.name}
                  </div>
